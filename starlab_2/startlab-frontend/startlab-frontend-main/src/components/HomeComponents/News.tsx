...
export default function News() {
  ...
  const sortedActiveNews = useMemo(() => {
  ...
  const top4 = sortedActiveNews.slice(0, 4);
  ...
  return (
    <section id="news" className="py-12 sm:py-16">
      <div className="px-4 sm:px-6 md:px-8">
        ...
        {/* ≤4 — как раньше сеткой, >4 — горизонтальный слайдер */}
        {!isLoading && !isError && activeNews.length > 0 && (
          <SnapCarousel
            threshold={4}
            ariaLabel="Список новостей"
            className="container-limited"
            arrowsOutside
          >
            {(activeNews.length > 4 ? sortedActiveNews : top4).map((n) => (
              <NewsItem
                key={n.id}
                title={n.title}
